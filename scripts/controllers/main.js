...
angular.module('codeHourApp')
  ...
  .controller('MainCtrl', ['$http', '$scope', 'FeedLoader', function($http, $scope, FeedLoader) 
  {
    $http.get('https://contesttrackerapi.herokuapp.com/android/')
      ...
      {
        var ongoings = data.result.ongoing;
        var upcomings = data.result.upcoming;
        var ongoing = [];
        var upcoming = [];
        for (var i = 0; i < ongoings.length; i++) 
        {
          ongoing.push({
             'name':ongoings[i].Name, 
             'url': ongoings[i].url,
             'platform':ongoings[i].Platform,
             'endTime': ongoings[i].EndTime,
          });
          $scope.ongoingcontests = ongoing;
        }
        for (var i = 0; i < upcomings.length; i++) 
        {
          upcoming.push({
            'name':upcomings[i].Name, 
            'url': upcomings[i].url,
            'platform':upcomings[i].Platform,
            'startTime': upcomings[i].StartTime,
            'endTime': upcomings[i].EndTime,
          }); 
          $scope.upcomingcontests = upcoming;
        }
      })
  }]);

// BELOW IS THE CODE WRITTEN BY SOMEONE LESSER THAN MYSELF, haha;
